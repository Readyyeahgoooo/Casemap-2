...
def _default_domain_label(domain_id: str) -> str:
...
def _domain_display_label(tree: dict, domain_id: str) -> str:
    label = str(tree.get("label_en") or _default_domain_label(domain_id))
...
def _coerce_domain_tree(payload: dict | list, domain_id: str) -> dict:
    domain_id = _normalize_domain_id(domain_id)
    ...
    label_en = str(tree.get("label_en") or tree.get("label") or _default_domain_label(domain_id))
    summary_en = str(
        tree.get("summary_en")
        or tree.get("summary")
...
def _generate_domain_tree(domain_id: str, output_path: Path) -> dict:
    ...
    domain_label = _default_domain_label(domain_id)
    prompt = (
...
def _iter_topics(tree: dict) -> list[dict]:
    topics: list[dict] = []
...
def _topic_catalog(tree: dict) -> list[dict]:
    topics: list[dict] = []
    for index, topic in enumerate(_iter_topics(tree), start=1):
...
def _storage_exports(
    ...
    *,
    ...
    chroma_path.write_text(
        json.dumps(
            {
                "collection": f"hk_{_normalize_domain_id(domain_id)}_cases",
