...
def _topic_catalog() -> list[dict]:
    topics: list[dict] = []
    for index, topic in enumerate(iter_criminal_topics(), start=1):
...
def _render_monitor_dashboard(title: str, progress: dict, report: dict | None = None) -> str:
    ...
    message = escape(str(progress.get("message", "Preparing criminal-law graph build.")))
...
<body>
  <main>
    <section class="hero">
      <h1>{escape(title)}</h1>
      <p>This monitor refreshes every {refresh_seconds // 60} minutes so you can leave it open while the criminal-law graph build continues.</p>
...
def _monitor_report(payload: dict, topic_catalog: list[dict], output_dir: Path, crawler_warnings: list[str], storage_status: dict) -> dict:
    ...
    report = {
        ...
        "next_actions": [
            "Run a second HKLII crawl on the uncovered topics using more specific offence or doctrine queries.",
            "Feed uncovered topics into NotebookLM or a DeepSeek/OpenRouter enrichment pass if an MCP or API becomes available.",
            "Review the low-coverage topics before promoting this graph as a full-production criminal-law map.",
        ],
    }
...
def _authority_tree_payload(payload_nodes: list[dict], payload_edges: list[dict], topic_catalog: list[dict]) -> dict:
...
def build_criminal_relationship_payload(
    ...
    if progress_callback:
        progress_callback(
            "searching_hklii",
            "Running seeded HKLII searches across the criminal-law topic catalog.",
            topic_count=len(topic_catalog),
    ...
    for source_index, source_path in enumerate(source_paths, start=1):
        ...
        if progress_callback:
            progress_callback(
                "loading_textbooks",
                f"Loaded source {source_index} of {len(source_paths)} for supplemental criminal-law coverage.",
                source_path_count=len(source_paths),
                loaded_source_count=len(supplemental_sources) - 1,
        ...
        add_node(
            {
                ...
                "label": source.label,
                "type": "source",
                "summary": f"{source.kind.upper()} textbook or note source used to fill criminal-law topic coverage.",
                "references": [],
                "links": [],
        ...
        add_node(
            {
                ...
                "links": [],
                "metrics": {},
                "keywords": top_keywords(f"{module['label_en']} {module['summary_en']}"),
            }
        )
    for topic in topic_catalog:
        add_node(
    ...
    if progress_callback:
        progress_callback(
            "assembling_graph",
            "Merging HKLII judgments, textbook passages, statutes, and topic links into one criminal-law payload.",
            fetched_case_count=len(case_documents),
            textbook_case_candidates=len(textbook_fetch_candidates),
            supplemental_passage_count=len(supplemental_passages),
            warning_count=len(crawler.warnings),
            ...
            add_node(
                {
                    ...
                    "type": "case",
                    "summary": "Authority cited inside an HKLII criminal-law judgment.",
                    "summary_en": "Authority cited inside an HKLII criminal-law judgment.",
                    "summary_zh": "",
    ...
    payload = {
        "meta": {
            ...
            "public_mode": True,
            "notes": [
                "Primary criminal-law judgments were fetched from HKLII's public API.",
            ...
            "lineage_codes": [],
            "viewer_heading_public": "Hong Kong Criminal Law Hierarchical Knowledge Graph",
    ...
    payload["meta"]["authority_tree"] = _authority_tree_payload(nodes, edges, topic_catalog)
...
def build_criminal_graph_artifacts(
    ...
    update_progress(
        "initializing",
        "Preparing criminal-law graph generation and monitor outputs.",
        source_path_count=len(source_paths),
        embedding_backend=embedding_backend,
    )
    payload, topic_catalog, sources, passages, case_documents, crawler_warnings = build_criminal_relationship_payload(
    ...
    if hybrid_output_dir:
        update_progress(
            "building_hybrid",
            "Building the hierarchical criminal-law explorer bundle.",
            node_count=len(payload["nodes"]),
            edge_count=len(payload["edges"]),
        )
    ...
    monitor = _monitor_report(payload, topic_catalog, relationship_output_path, crawler_warnings, storage_status)
    ...
    update_progress(
        "completed",
        "Criminal-law graph build completed. Monitor report and exports are ready.",
        status="completed",
